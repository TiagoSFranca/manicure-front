<template>
  ...
  >
    <v-card-title>
      <span class="overline">{{ $t(PRODUCT.CARD_IMAGES.NAME) }}</span>
    </v-card-title>
    <v-card-text>
      <v-row>
        <v-spacer></v-spacer>
        <v-col cols="4">
          <common-file-upload
            ...
            :label="$t(PRODUCT.CARD_IMAGES.LABELS.ADD_IMAGE)"
    ...
    <common-confirm-dialog
      ...
      :title="$t(PRODUCT.CARD_IMAGES.MESSAGES.CONFIRM_DELETE.TITLE)"
      :message="$t(PRODUCT.CARD_IMAGES.MESSAGES.CONFIRM_DELETE.MESSAGE)"
