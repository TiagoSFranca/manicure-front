          ...
          <v-card-text>
            <v-form>
              <v-row v-if="type !== SALE_STATUS.END">
                <v-col cols="4" v-if="type === SALE_STATUS.START">
                  <validation-provider
                    :rules="`required|greater_than:0|lower_than_other:${price},'${$t(
                      PRODUCT.CHANGE_SALE.LABELS.PRICE
                    )}'`"
                    v-slot="{ errors }"
                  >
                    <v-currency-field
                      :label="$t(PRODUCT.CHANGE_SALE.LABELS.PROMOTIONAL_PRICE)"
                ...
                <v-col cols="6">
                  ...
                  >
                    <common-date-picker
                      :date="object.date"
                      :label="$t(PRODUCT.CHANGE_SALE.LABELS.END_SALE)"
              ...
              <v-row v-else>
                <v-col
                  ><span class="overline">{{
                    $t(PRODUCT.CHANGE_SALE.MESSAGES.CONFIRM_END)
                  }}</span></v-col
                >
              </v-row>
            </v-form>
...
export default {
  props: ["showChangeSale", "type", "price", "idProduct", "date"],
  data() {
    return {
      visible: false,
      menu: false,
      ...
      LOADING_IDENTIFIER: "changeProductSale",
    ...
    getTitle() {
      switch (this.type) {
        case SALE_STATUS.START:
          return this.PRODUCT.CHANGE_SALE.START_NAME;
        case SALE_STATUS.END:
          return this.PRODUCT.CHANGE_SALE.END_NAME;
        case SALE_STATUS.ANTECIPATE:
          return this.PRODUCT.CHANGE_SALE.ANTECIPATE_NAME;
        case SALE_STATUS.EXTEND:
          return this.PRODUCT.CHANGE_SALE.EXTEND_NAME;
      }
    },
    setDate() {
