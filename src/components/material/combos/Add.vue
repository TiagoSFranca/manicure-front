<template>
  <v-row justify="center">
    <v-dialog v-model="visible" scrollable persistent max-width="800px">
      <validation-observer ref="form" v-slot="{ handleSubmit }">
        <v-card :disabled="loading[LOADING_IDENTIFIER]">
          <v-card-title>
            <span class="headline">{{ $t(COMBO.ADD.NAME) }}</span>
          </v-card-title>
          <v-card-text>
            <v-form>
              <v-row>
              ...
              <v-row align="start">
                <v-col cols="4">
                  ...
                  >
                    <v-currency-field
                      :label="$t(COMBO.ADD.LABELS.PRICE)"
                ...
                <v-col cols="4">
                  <validation-provider
                    :rules="`${
                      ...
                      COMBO.ADD.LABELS.PRICE
                    )}'`"
                    v-slot="{ errors }"
                  >
                    <v-currency-field
                      :label="$t(COMBO.ADD.LABELS.PROMOTIONAL_PRICE)"
                ...
                <v-col cols="4">
                  <validation-provider v-slot="{ errors }">
                    <v-checkbox
                      ...
                      :label="$t(COMBO.ADD.LABELS.ON_SALE)"
                      color="primary"
                      hide-details
                      :error-messages="errors"
                    ></v-checkbox>
              ...
              <v-row align="start">
                <v-col cols="4">
                  <validation-provider v-slot="{ errors }">
                    <v-checkbox
                      ...
                      :label="$t(COMBO.ADD.LABELS.ACTIVE)"
                      color="primary"
                      hide-details
                      :error-messages="errors"
                    ></v-checkbox>
                ...
                <v-col cols="4">
                  <common-date-picker
                    ...
                    :label="$t(COMBO.ADD.LABELS.END_SALE)"
              ...
              <v-row>
                <v-col cols="12">
                  <validation-provider rules="max:512" v-slot="{ errors }">
                    <v-textarea
                      :label="$t(COMBO.ADD.LABELS.COMMENTS)"
    ...
    <common-confirm-dialog
      ...
      :title="$t(COMBO.ADD.MESSAGES.REDIRECT_TO_EDIT.TITLE)"
      :message="$t(COMBO.ADD.MESSAGES.REDIRECT_TO_EDIT.MESSAGE)"
    ...
    >
      <template slot="actions">
        ...
        <v-btn
          ...
          icon
          fab
          :to="{ name: COMBOS_EDIT.name, params: { id: idCombo } }"
        >
          <v-icon>mdi-check</v-icon>
        </v-btn>
      </template>
...
export default {
  ...
  data() {
    return {
      ...
      LOADING_IDENTIFIER: "addCombo",
      idCombo: "",
      COMBOS_EDIT: COMBOS_EDIT
    };
  },
  methods: {
    hide() {
    ...
    save() {
      combosActions.add(this.object, this.LOADING_IDENTIFIER).then((res) => {
        if (res.success) {
